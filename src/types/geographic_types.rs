...
impl Constituency {
    pub fn from_record(record: &StringRecord, headers: &StringRecord) -> Self {
        ...
        let id_string = record.get(column_map["ONS ID"]).expect("Missing ONS ID").to_string();
        let name_string = record.get(column_map["Constituency name"]).expect("Missing Constituency Name").to_string();
        let country_string = record.get(column_map["Country name"]).expect("Missing Country Name");
