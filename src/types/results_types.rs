...
use crate::types::party_types::{Parties, Party, PARTY_COLUMNS};
...
impl ConstituencyResult {
    pub fn from_record(record: &StringRecord, headers: &StringRecord) -> Self {
        ...
        let winner_string = record
            .get(column_map["First party"])
            .expect("Missing Winning Party");
        let winner = Party::from_string(winner_string);

        let outcome_string = record
            .get(column_map["Result"])
            .expect("Missing Result")
            .to_string();
        let majority_string = record
            .get(column_map["Majority"])
            .expect("Miossing Majority");
        let majority = majority_string
            .parse::<i64>()
            .expect("Invalid vote count for Majority");

        let mut party_results: Vec<PartyResult> = Vec::new();
        let mut total_other_votes = 0;
        for party_col in PARTY_COLUMNS {
            let party_votes_string = record
                .get(column_map[party_col])
                .expect(&format!("Missing Party {}", party_col));
            let party_votes = party_votes_string
                .parse::<i64>()
                .expect(&format!("Invalid vote count for {}", party_col));
